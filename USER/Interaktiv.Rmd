...
ShowTS(data = data, # Name of data object
       ...
       extraval = "Tot_hjerneslag", # e.g. "Dod_med_hjerteinfarkt"
       tab = F) # Prints table, change to FALSE if you only want the plot
```

# Plot multiple time series

- Useful if you want to see the time series for all age groups, all genders, all diagnoses etc for one strata. 
```{r message=FALSE, warning=FALSE}
# Creates one plot per unique level of the selected dimension
# All inputs are the same as above
walk(unique(data$KJONN), # Select dimension you want to plot separately, insert .x as value below on the corresponding row
    ~ShowTS(data = data, 
            maltall = "MEIS", 
            geo = 030115, 
            alder = NULL, 
            kjonn = .x, # Insert .x for the variable selected above
            utdann = NULL, 
            innvkat = NULL, 
            landbak = NULL, 
            extradim = "KODEGRUPPE", # If the selected dimension is the extradimension, extraval must be .x
            extraval = "Tot_hjerneslag", 
