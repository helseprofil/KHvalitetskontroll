...
```{r echo = F}
## Load new data file
dfnew <- ReadFile(file = "TRANGBODDHET_2022-12-12-13-36", # Matching, if >1 file found in folder, specify file name further
                  modus = "KH", # KH or NH
...
### 
# Change column names (if different names for same column, especially relevant for NH-2022)
# NB: for the HTML report, all name changes must remain uncommented when saving the report. 

# setnames(dfold, old = "antall", new = "TELLER")
# setnames(dfold, old = "Crude", new = "RATE")
# setnames(dfold, old = "Adjusted", new = "MEIS")
# setnames(dfold, old = "sumteller", new = "sumTELLER")
# setnames(dfold, old = "sumnevner", new = "sumNEVNER")

# Or do all at once (all "old" columns must be present)
# setnames(dfold, 
#          old = c("antall", "Crude", "Adjusted", "sumteller", "sumnevner"),
#          new = c("TELLER" ,"RATE", "MEIS", "sumTELLER", "sumNEVNER"))
```
...
CompareBydelKommune(groupdim = c(EXTRAdims, "ALDER", "KJONN"))
```
...
CompareOslo(groupdim = c(EXTRAdims, "ALDER", "KJONN"))
```
