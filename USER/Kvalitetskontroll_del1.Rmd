...
```{r echo = F}
## Load data
dfnew <- ReadFile(file = "TRANGBODD", # Matching, if >1 file found in folder, specify file name further
                  modus = "KH", # KH or NH
                  folder = 2023) # "DATERT" or 4-digit year if file in NESSTAR-folder

dfold <- ReadFile(file = "TRANGBODD", # Matching, if >1 file found in folder, specify file name further
                  modus = "KH",# KH or NH
...
STANDARDdims <- c("GEO", "AAR", "KJONN", "ALDER", "UTDANN", "INNVKAT") 
...
GROUPdims <- c("BODD", "AAR", "ALDER") 

# Time series plots
# Generates one plot per PLOTDIMS, and each plot contains a panel per PLOTTVALS
# Currently accepts the following dimensions: "KJONN", "UTDANN", "INNVKAT", "LANDBAK"
# Totals are kept for dimensions other than the one being plotted
PLOTDIMS <- c("UTDANN")
