...
```{r echo = F}
## Load data
dfnew <- ReadFile(file = "HKR_2022-11-30", # Matching, if >1 file found in folder, specify file name further
                  modus = "KH", # KH or NH
...
- In the compareKUBE, calculate per value column per geographical level:
    - How many rows are identical (same value or both censored)
    - How many rows are different (different value, or one value censored)
    - How many rows are censored in the new file, but not in the old (N new prikk)
    - How many rows were censored in the old file, but no longer in the new (N expired prikk)
    - Mean, minumum and maximum difference between new and old file
```{r}
