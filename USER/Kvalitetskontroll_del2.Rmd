...
VALUES <- c("TELLER", "RATE", "SMR", "MEIS", "sumTELLER", "sumNEVNER", "RATE.n", "SPVFLAGG")
...
data %>% 
  filter(GEO == 0) %>% 
  group_by_("AAR", dim) %>% 
  summarise(TELLER = sum(TELLER, na.rm = T), .groups = "drop")} %>% 
  ggplot(aes(x = AAR,
             y = TELLER,
             group = dim,
             color = dim)) +
...
totalsonly <- DIMENSIONS[!DIMENSIONS %in% c("GEO", "AAR")]
...
dfnew %>% 
  filter(if_all(all_of(totalsonly), ~ . == 0))


dfnew %>% filter(KJONN == 0, ALDER == 0)
